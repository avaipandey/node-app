         ...
         stage('Deploy to GKE Docker Image'){
            steps{
                 withCredentials([file(credentialsId: 'jacc', variable: 'gke')]) {
                     ...
                     sh "gcloud compute scp --strict-host-key-checking=no ~/serivces.yml ~/pods.yml jenkins1@robotic-tract-277114.iam.gserviceaccount.com:~/jenkins1/ --zone=us-central1-a"
                     sh "kubectl apply -f"
                  }
