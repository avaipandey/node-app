         ...
         stage('Deploy to GKE Docker Image'){
            steps{
                 withCredentials([file(credentialsId: 'jacc', variable: 'gke')]) {
                     ...
                     sh "gcloud compute scp --strict-host-key-checking=no ~/serivces.yml ~/pods.yml jenkins1@gke-cluster-1-default-pool-8f7b19f0-8fpl:/home/jenkins1/ --zone=us-central1-a"
                     sh "kubectl apply -f"
                  }
